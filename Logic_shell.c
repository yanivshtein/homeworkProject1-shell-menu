...
int main(int argc, char* argv[]){
	char cmdLine[4], str[100], args[2][30], file[40], charLine[10];
	int flag = 0, fd_to, i, line, amountCommands = 0;
	...
	if(opendir("Commands/Logic") == NULL){
		if(mkdir("Commands/Logic", 0775) == -1){
	...
	while(1){
		...
		// Write the line number and command to the file
		...
		if(strcmp(args[0],"Highbit") == 0 && strcmp(args[1],"") != 0 && amountCommands < 3){
			flag = 1;
			if(fork() == 0){
				execl(args[0],args[0],args[1],NULL);
				printf("executing Highbit failed\n");
				exit(1);
			}
		}
		if(strcmp(args[0],"DectoBin") == 0 && strcmp(args[1],"") != 0 && amountCommands < 3){
			flag = 1;
			if(fork() == 0){
				execl(args[0],args[0],args[1],NULL);
				printf("executing DectoBin failed\n");
				exit(1);
			}
		}
		if(strcmp(args[0],"DectoHex") == 0 && strcmp(args[1],"") != 0 && amountCommands < 3){
			flag = 1;
			if(fork() == 0){
				execl(args[0],args[0],args[1],NULL);
				printf("executing DectoHex failed\n");
				exit(1);
			}
		}
		if(strcmp(args[0],"History") == 0 && strcmp(args[1],"") == 0){
			flag = 1;
			if(fork() == 0){
				execl("History","History","2",NULL);
